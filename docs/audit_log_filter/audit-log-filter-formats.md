...
| JSONL | `audit_log_filter.format=JSONL` | Each event is a single compact JSON object on one line ([JSON Lines](https://jsonlines.org/) format). Easy to process with `grep`, `jq`, `wc -l`, streaming pipelines, and log aggregation systems. |
